...
pub struct ResistanceTemperatureDetector {
	pub resistance_at_0c: f32,
}

impl ResistanceTemperatureDetector {
	pub fn convert_resistance_to_temperature(
		...
		let resistance_ratio = measured_resistance / self.resistance_at_0c;
		...
		if discriminant >= 0.0 {
			let temperature_celsius = (-A + discriminant.sqrt()) / (2.0 * B); // B < 0
		...
		// Perform a small, fixed number of iterations. 5 is typically plenty
		...
		for _ in 0..5 {
			...
			let predicted_measured_resistance = self.resistance_at_0c * (1.0 + A * t + B * t2 + C * (t - 100.0) * t3);
			...
			let derivative_ohms_per_c = self.resistance_at_0c * (A + 2.0 * B * t + C * (4.0 * t3 - 300.0 * t2));
