...
pub async fn sm_task(
	spawner: Spawner,
	mut state_machine: StateMachine<Context>,
) {
		...
		match state_machine.state {
			States::Init => {
				...
				let msg = messages_prost::radio::RadioFrame {
					...
					payload: Some(messages_prost::radio::radio_frame::Payload::PhoenixState(State::Init.into())),
			...
			States::Idle => {
				...
				let msg = messages_prost::radio::RadioFrame {
					node: messages_prost::common::Node::Phoenix.into(),
					payload: Some(messages_prost::radio::radio_frame::Payload::PhoenixState(State::Idle.into())),
					millis_since_start: Instant::now().as_millis(),
				};
				msg.encode_length_delimited(&mut buf.as_mut()).unwrap();
				SD_CHANNEL.send(("state.txt", buf)).await;
			...
			States::Calibration => {
				...
				let msg = messages_prost::radio::RadioFrame {
					node: messages_prost::common::Node::Phoenix.into(),
					payload: Some(messages_prost::radio::radio_frame::Payload::PhoenixState(State::Calibration.into())),
					millis_since_start: Instant::now().as_millis(),
				};
				msg.encode_length_delimited(&mut buf.as_mut()).unwrap();
				SD_CHANNEL.send(("state.txt", buf)).await;
			...
			States::Collection => {
				...
				let msg = messages_prost::radio::RadioFrame {
					node: messages_prost::common::Node::Phoenix.into(),
					payload: Some(messages_prost::radio::radio_frame::Payload::PhoenixState(State::Collection.into())),
					millis_since_start: Instant::now().as_millis(),
				};
				msg.encode_length_delimited(&mut buf.as_mut()).unwrap();
				SD_CHANNEL.send(("state.txt", buf)).await;
