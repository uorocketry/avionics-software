...
use argus::ads::register_data;
use argus::ads::{Ads1262, Command, Register};
use argus::resources::SD_CHANNEL;
use argus::resources::{ADC_SPI_BUS_CELL, HEAP};
use argus::state_machine::{sm_task, StateMachine};
use defmt::*;
use defmt_rtt as _;
...
pub static RTC: Mutex<CriticalSectionRawMutex, RefCell<Option<Rtc>>> = Mutex::new(RefCell::new(None));
...
fn adc_to_celsius(adc_value: u16) -> f32 {
	...
	let steinhart = logf(resistance / THERMISTOR_NOMINAL_RESISTANCE) / THERMISTOR_BETA + (1.0 / TEMPERATURE_NOMINAL_KELVIN);
		...
		if let Ok((_status, raw_data)) = data {
			...
			#[cfg(feature = "temperature")]
			{
				...
				if let Some(celsius) = thermocouple_converter::voltage_to_celsius(volts) {
					...
					msg.encode_length_delimited(&mut buf.as_mut()).expect("Failed to encode SBG GPS Position");
			...
			#[cfg(feature = "pressure")]
			{
				...
				let msg = messages_prost::radio::RadioFrame {
					...
					payload: Some(messages_prost::radio::radio_frame::Payload::ArgusPressure(
						messages_prost::sensor::argus::Pressure { sensor_id, pressure },
					)),
					millis_since_start: Instant::now().as_millis(),
				};
				msg.encode_length_delimited(&mut buf.as_mut()).expect("Failed to encode SBG GPS Position");
...
async fn main(spawner: Spawner) {
	...
	let sd_card: embedded_sdmmc::SdCard<
		embedded_hal_bus::spi::RefCellDevice<'static, Spi<'static, embassy_stm32::mode::Blocking>, Output<'static>, Delay>,
		Delay,
	...
	#[cfg(not(feature = "calibration"))]
	{
		...
		adc1.write_registers(Register::OFCAL0, &[adc1_ofcal_bytes[1], adc1_ofcal_bytes[2], adc1_ofcal_bytes[3]])
			.unwrap();

		let adc1_fscal_bytes = ADC1_FSCAL.to_be_bytes();
		adc1.write_registers(Register::FSCAL0, &[adc1_fscal_bytes[1], adc1_fscal_bytes[2], adc1_fscal_bytes[3]])
			.unwrap();

		let adc2_ofcal_bytes = ADC2_OFCAL.to_be_bytes();
		adc2.write_registers(Register::OFCAL0, &[adc2_ofcal_bytes[1], adc2_ofcal_bytes[2], adc2_ofcal_bytes[3]])
			.unwrap();

		let adc2_fscal_bytes = ADC2_FSCAL.to_be_bytes();
		adc2.write_registers(Register::FSCAL0, &[adc2_fscal_bytes[1], adc2_fscal_bytes[2], adc2_fscal_bytes[3]])
			.unwrap();

		info!("ADC1 OFCAL: {:#08x}, FSCAL: {:#08x}", ADC1_OFCAL, ADC1_FSCAL);
		info!("ADC2 OFCAL: {:#08x}, FSCAL: {:#08x}", ADC2_OFCAL, ADC2_FSCAL);
	}
