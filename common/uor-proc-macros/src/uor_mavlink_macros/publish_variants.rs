...
fn __value_mode_stream(
	...
	quote! {
		impl crate::uor_mavlink::uor_mavlink_communications_traits::publisher::Publisher for #name {
		fn publish(
			&mut self,
			buff: &mut [u8],
			sequence: u8,
		) -> usize {
			let mut frame = crate::uor_mavlink::uor_mavlink_dialect::MAVLinkV2MessageRaw::new();
			let header = crate::uor_mavlink::uor_mavlink_dialect::MavHeader {
				system_id: 1,
				component_id: 1,
				sequence: sequence,
			};
...
fn __buffered_mode_stream(
	...
	quote! {
		impl<const #generic_ident: usize> crate::uor_mavlink::uor_mavlink_communications_traits::publisher::Publisher for #name<#generic_ident> {
		fn publish(
			&mut self,
			buff: &mut [u8],
			sequence: u8,
