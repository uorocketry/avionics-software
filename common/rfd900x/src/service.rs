...
impl RFD900XService {
    pub async fn new(io_service: RingBufferedSerialService, config: Config) -> RFD900XService {
    ...
    pub async fn write_all(&mut self, data: &[u8]) -> Result<(), RFD900XError> {
    ...
    async fn write_register_with_delay(&mut self, register_number: Registers, value: u32) {
    ...
    pub async fn write_config(&mut self) -> Result<(), ConfigurationError> {
        ...
        self.write_register_with_delay(Registers::SerialSpeed, config.serial_speed as u32)
            .await;
        self.write_register_with_delay(Registers::AirSpeed, config.air_speed as u32)
            .await;
        self.write_register_with_delay(Registers::NetId, config.net_id as u32)
            .await;
        self.write_register_with_delay(Registers::TxPower, config.tx_power as u32)
            .await;
        self.write_register_with_delay(Registers::ECC, config.ecc as u32)
            .await;
        self.write_register_with_delay(Registers::Mavlink, config.mavlink as u32)
            .await;
        self.write_register_with_delay(Registers::OpResend, config.op_resend as u32)
            .await;
        self.write_register_with_delay(Registers::MinFreq, config.min_freq)
            .await;
        self.write_register_with_delay(Registers::MaxFreq, config.max_freq)
            .await;
        self.write_register_with_delay(Registers::NumChannels, config.num_of_channels as u32)
            .await;
        self.write_register_with_delay(Registers::DutyCycle, config.duty_cycle as u32)
            .await;
        self.write_register_with_delay(Registers::LBTRSSI, config.lbt_rssi as u32)
            .await;
        self.write_register_with_delay(Registers::MaxWindow, config.max_window as u32)
            .await;
    ...
    async fn write_register(&mut self, register_number: Registers, value: u32) {
        ...
        payload[ATS_OFFSET..ATS_OFFSET + register_digit_offset]
            .copy_from_slice(register_component.as_bytes());

        payload[ATS_OFFSET + register_digit_offset] = b'=';
        register_addressing_offset = ATS_OFFSET + register_digit_offset + EQUAL_SIGN_OFFSET;

        payload[register_addressing_offset..register_addressing_offset + value_digit_offset]
            .copy_from_slice(value_component.as_bytes());
    ...
    pub fn split(self) -> (RFD900Tx, RFD900Rx) {
        ...
        return (
            RFD900Tx {
                component: io_split.0,
            },
            RFD900Rx {
                component: io_split.1,
            },
        );
    }
}
...
impl embedded_io_async::Read for RFD900XService {
    async fn read(&mut self, buf: &mut [u8]) -> Result<usize, RFD900XError> {
    ...
    async fn read_exact(
        ...
        match self.io_service.read_exact(buf).await {
            ...
            Err(e) => {
                return Err(embedded_io_async::ReadExactError::Other(
                    RFD900XError::ReadError,
                ));
            }
        }
    }
...
impl embedded_io_async::Write for RFD900XService {
    async fn write(&mut self, buf: &[u8]) -> Result<usize, RFD900XError> {
