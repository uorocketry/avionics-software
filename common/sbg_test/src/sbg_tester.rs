...
use embedded_io_async::{Read, Write, ErrorType, Error};
use sbg_hal::sbg_device::{SbgDevice};
use sbg_hal::data_structs::messages::SbgEcomLogMag;
...
impl SbgTester {
	...
	pub fn new(internal_buffer_1: &[u8], internal_buffer_2: &[u8]) -> SbgTester {
		...
		SbgTester { internal_buffer_1: intermediate_1, internal_buffer_2: intermediate_2, write_buffer : [0; 4096], read_count: 0}
	}
}
...
impl Read for SbgTester {
	...
	async fn read(&mut self, buf: &mut [u8]) -> Result<usize, ErrorSbg> {
		let mut index = 0;
