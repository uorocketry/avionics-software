...
use syn::{
    Data, DeriveInput, Fields, GenericArgument, GenericParam, Meta, PathArguments, Type,
    parse_macro_input,
};
// # set(CPM_mavlink_SOURCE "C:/Users/tubro/Desktop/Avionics/avionics-software/uor_mavlink")
...
pub fn derive_publish(input: TokenStream) -> TokenStream {
    ...
    if let Data::Struct(struct_data) = &input.data {
        ...
        return TokenStream::from(
            syn::Error::new(
                input.ident.span(),
                "This derive macro must be applied to a struct",
            )
            .to_compile_error(),
        );
    }
    ...
    if generic_ident.is_none() && is_buffered {
        return TokenStream::from(
            syn::Error::new(
                input.ident.span(),
                "This struct lacks the buffer size constant generic",
            )
            .to_compile_error(),
        );
    }
    ...
    if burst_mode && is_buffered {
        output = __buffered_burst_mode_stream(
                    ...
                    if let PathArguments::AngleBracketed(arguments) = x.arguments.clone() {
                        ...
                        if let None = arg {
                            return TokenStream::from(
                                syn::Error::new(
                                    input.ident.span(),
                                    "Array type is malformed. Insure that the type is defined",
                                )
                                .to_compile_error(),
                            );
                        }
