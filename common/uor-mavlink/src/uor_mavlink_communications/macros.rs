...
macro_rules! initialize_publisher_pool {
	($num_of_tasks:literal) => {
		pub fn start_publishers(
			publishers: &'static mut [&'static utils::types::AsyncMutex<
				dyn uor_mavlink::uor_mavlink_communications_traits::publisher::PeriodicPublisher,
			>],
			task_scheduler: &Spawner,
			mavlink: &'static utils::types::AsyncMutex<uor_mavlink::uor_mavlink_service::service::MavlinkServiceTx>,
		) {
		...
		async fn __start_publisher(
			publisher: &'static utils::types::AsyncMutex<dyn uor_mavlink::uor_mavlink_communications_traits::publisher::PeriodicPublisher>,
			mavlink: &'static utils::types::AsyncMutex<uor_mavlink::uor_mavlink_service::service::MavlinkServiceTx>,
		) {
			loop {
				let mut delay;
	...
	() => {
		pub fn start_subscribers(
			subscribers: &'static mut [&'static utils::types::AsyncMutex<
				dyn uor_mavlink::uor_mavlink_communications_traits::subscriber::Subscriber,
			>],
			task_scheduler: &Spawner,
