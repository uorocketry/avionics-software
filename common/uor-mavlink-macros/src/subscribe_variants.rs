...
pub fn subscriber_stream(
	...
	quote! {
			...
			impl<const #generic_ident: usize> crate::uor_mavlink_communications_traits::subscriber::Subscriber for #name<#generic_ident> {
				fn __update(
					self: &mut Self,
					frame: crate::uor_mavlink_dialect::MAVLinkV2MessageRaw,
					)
				{
				// Check if the subscriber subscribes to the frame
				if frame.message_id() == #name_of_type::ID {
					let payload_raw = frame.payload();
					let payload_deserialized = #name_of_type::deser(crate::uor_mavlink_dialect::MavlinkVersion::V2, payload_raw);
