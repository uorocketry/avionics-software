...
use heapless::String;
use uor_peripherals::serial_ring_buffered::peripheral::{RingBufferedUORSerial, RingBufferedUORSerialRx, RingBufferedUORSerialTx};
...
pub struct RFD900X {
	io_service: RingBufferedUORSerial,
	config: Config,
}
...
impl ErrorType for RFD900X {
	type Error = RFD900XError;
}

impl embedded_io_async::Read for RFD900X {
	async fn read(
		&mut self,
		buf: &mut [u8],
...
impl embedded_io_async::Write for RFD900X {
	async fn write(
		&mut self,
		buf: &[u8],
	) -> Result<usize, RFD900XError> {
...
pub struct RFD900Tx {
	pub component: RingBufferedUORSerialTx,
}
...
pub struct RFD900Rx {
	pub component: RingBufferedUORSerialRx,
}
